...
function decodeRawImage(
  ...
  try {
    switch (encoding) {
      ...
      case "16UC1": {
        for (let i = 0; i < width * height; i++) {
          ...
          const normalized =
            (value16 - config.valueMin) / (config.valueMax - config.valueMin);
      ...
      case "32FC1": {
        ...
        for (let i = 0; i < width * height; i++) {
          ...
          const normalized =
            (value32 - config.valueMin) / (config.valueMax - config.valueMin);
    ...
    return {
      ...
      data: transformed.data.buffer as ArrayBuffer,
      encoding,
    };
  } catch (error) {
...
function handleDecodeRaw(
  ...
  if (decoded) {
    ...
    self.postMessage(response, { transfer: [decoded.data] });
  } else {
